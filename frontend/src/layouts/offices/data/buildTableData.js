...
const buildOfficeTableData = (officeList) => {
  ...
  const Manager = ({ manager }) => (
    <MDBox display="flex" alignItems="center" lineHeight={1}>
      <MDAvatar src={team2} name={manager ? manager.name : ""} size="sm" />
      <MDBox ml={2} lineHeight={1}>
        <MDTypography display="block" variant="button" fontWeight="medium">
          {manager ? manager.name : ""}
        </MDTypography>
        <MDTypography variant="caption">{manager ? manager.mobilePhoneNumber :  "" }</MDTypography>
      </MDBox>
    </MDBox>
  );
  const Address = ({ address }) => (
    <MDBox display="flex" alignItems="center" lineHeight={1}>
      <MDBox ml={2} lineHeight={1}>
        ...
        <MDTypography variant="caption">{address.postalCode}, {address.City}</MDTypography>
      </MDBox>
    </MDBox>
  );
  ...
  return {
    columns: [
      { Header: "office name", accessor: "officename", width: "30%", align: "left" },
      { Header: "office Address", accessor: "officeAddress", align: "left" },
