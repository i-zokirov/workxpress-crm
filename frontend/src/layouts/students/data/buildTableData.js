...
const buildTableData = (studentsList) => {
  ...
  return {
    columns: [
      ...
      { Header: "registered by", accessor: "registeredby", align: "left" },
    ...
    rows: studentsList.map(student => {
      return {
        student: <Student image={team2} name={student.name} email={student.email} />,
        mobile: (
          <MDTypography variant="caption" color="text" fontWeight="medium">
            {student.mobilePhoneNumber}
          </MDTypography>
        ),
        registeredby: <Creator name={student.createdBy.name} role={student.createdBy.role} />,
        status: (
          <MDBox ml={-1}>
            <MDBadge badgeContent={student.status} color={assignColor(student.status)} variant="gradient" size="sm" />
          </MDBox>
        ),
        registered: (
          <MDTypography variant="caption" color="text" fontWeight="medium">
        ...
        age: (
          <MDTypography  variant="caption" color="text" fontWeight="medium">
            {new Date().getFullYear() - student.birthdate.split("/")[2]}
          </MDTypography>
        ),
        action: (
          <MDTypography component={Link} to={`/students/${student._id}`} variant="caption" color="text" fontWeight="medium">
            View
          </MDTypography>
        ),
      }
